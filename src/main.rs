...
/// Return the arguments GTK should see, with our own flag (`-v`) removed so
...
mod tests {
    ...
    /// `gtk_args` strips our own flag (`-v`) and passes every other argument
    /// through in order — including unknown flags, which GTK itself reports.
    #[rstest]
    ...
    #[case::passes_real_gtk_flag("simpletaskmgr --version", "simpletaskmgr --version")]
    #[case::passes_unknown_flag(
        "simpletaskmgr --totally-unknown -v",
        "simpletaskmgr --totally-unknown"
    )]
