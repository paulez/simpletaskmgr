...
**The honest caveat (read this before "it already failed, why retry?").**
This is *functionally nearly a re-implementation* of the v1 attempt
(`81cc6ac`) — which the log says was user-tested **and still showed the
desync**, so it was reverted. The shape is the same: park the
`glib::Binding`, `unbind()` it on recycle. What changed is the *context*
around that shape, and we cannot cleanly attribute the fix to a single one
of the three items above:

- v1 carried `unsafe` qdata; this version does not. (Likely not the cause
  of the functional difference, but it is now mergeable.)
- v1 and this both retire in `connect_unbind` + defensively in `connect_bind`
  — but the *Phase 0 baseline* that v1 was reverted to had **no** retirement
  at all (leaked C-side binding, `BIND_PID_KEY` i32 only). The state the user
  is now testing is the first one that retires in both paths **and** has
  `set_incremental(false)` removed **simultaneously**.
- The bug was intermittent. "Does not reproduce now" can also be explained by
  the user's session not hitting the specific kworker-churn pattern, or by
  `set_incremental(false)` removal alone.

So the *record* should read: **user-confirmed fixed (2026-09-08)** — that
is the operational fact — with the caveat that we did not A/B test the three
changes in isolation, and the prior "v1 already failed" data point is not
overwritten, merely superseded in *this specific combination*. If it ever
recurs, the Phase 1 diagnostics (still in the tree) are the instrument we
will frame a repro log against; do not assume this fix on its own and
strip them.

The test caveat is separate and stands: a **green test suite does not prove
the fix in the running app.** My `cell_label.rs` tests prove the slab retires
a binding in isolation (a stale source's `notify` no longer reaches the
target once `take_and_unbind` runs). They do *not* exercise GTK's real
