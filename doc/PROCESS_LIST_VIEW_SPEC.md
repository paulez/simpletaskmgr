...
| D6 | **Full command popover**: the `Command` row shows a long value as a single bounded (ellipsized) line; a single click on that row toggles a popover anchored to the row that reveals the selected process' **full** command line in a read-only, selectable (copyable), horizontally-scrolled text view that wraps very long tokens mid-word. The popover tracks its revealed PID: it stays open when a refresh re-applies the same selection, and closes when the selection moves to a different process or is cleared. The row itself is *not* selectable — a selectable `GtkLabel` installs its own exclusive click gesture that would suppress the toggle — so the full command is copied from the popover's selectable text. The full value is also available in a hover tooltip. |
