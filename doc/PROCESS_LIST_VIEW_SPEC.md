...
| D2 | Selecting a row shows a detail pane with **PID, Name, Command line, State, Threads, Nice, PPID, CPU %, MEM %, Memory, Disk read, Disk write, Started, Uptime** for that process, formatted as in a row cell (V2) with unknown values shown as `—`. The command line is selectable (copy/paste). The start time and uptime are derived from `stat.starttime` (ticks since boot) plus the host's `/proc/uptime` and ticks/sec; the absolute Memory is the RSS in a human unit. Originally only PID/Name/CPU/MEM/Disk were shown; the rest are added for 1.0.0. |
